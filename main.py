...
def init_and_create_directory(
    path: str, fixed_name: Optional[str] = None, clear: bool = False
) -> Optional[str]:
    """
    ...
    dir_name = (
        fixed_name
        if fixed_name
        else f'{Haikunator().haikunate()}-{datetime.now().strftime("%Y%m%d%H%M%S")}'
    )
...
def fetch_data(bucket_name: str, file_name: str, raw_data_path: str, run) -> None:
    ...
    s3_client.download_file(
        bucket_name, file_name, f"{raw_data_path}/{local_file_name}"
    )
...
@task
def preprocess_data(
    raw_data_path: str, save_path: str, spark_master: str, spark_app_name: str, run
) -> None:
    """
    Preprocess the data
    ...
    spark = (
        SparkSession.builder.master(spark_master).appName(spark_app_name).getOrCreate()
    )
    ...
    preprocessed_data.write.parquet(path_preprocessed_data, mode="overwrite")
...
@task
def data_pipeline(
    feature_engineered_data_path: str,
    path_proc_data_path: str,
    path_pipeline_model: str,
    spark_master: str,
    spark_app_name: str,
    run,
) -> None:
...
@task
def init_model_experiment(
    config_file: str, max_evals: int, seed: int
) -> ModelExperiment:
    ...
    base_classifiers = [(name, deepcopy(clf)) for name, clf in base_class.items()]
    ...
    for model, params in config["hyperparameters"].items():
        ...
        for param, values in params.items():
            ...
            hyperparameters[model][param] = getattr(hp, values["type"])(
                param, *map(float, values["args"])
            )

    # Initialize the model experimenter
    model_experimenter = ModelExperiment(
        classifiers=classifiers,
        spaces=hyperparameters,
        max_evals=max_evals,
        seed=seed,
        workdir=config["dirs"]["session_dir"],
    )
...
def run_model_calibration(model_experimenter: ModelExperiment, proc_data: str):
    ...
    results = model_experimenter.run_calibration_exp(X_train, y_train, X_test, y_test)
...
def model_explainability(session_dir: str, n_samples: int):
    ...
    for model_name in [
        m for m in os.listdir(models_path) if not m.startswith("calibrated")
    ]:
        print(model_name)
        ...
        explainer.plot_shap_force(sample_index=0, output_index=0, path=artifacts_path)
...
def log_wandb_artifact(artifact_name: str, dir_path: str, models_path: str, run):
    ...
    def extract_model_name(file_name: str) -> str:
        ...
        Returns:
            str: The model name.
        """
        prefixes = ["confusion_matrix", "calibration_comparison", "pr_roc_curves"]
...
def sparkify_flow():
    ...
    init_and_create_directory(path=base_dir, fixed_name=session_name, clear=True)
    ...
    feature_engineering(
        path_preprocessed_data=preprocessed_data,
        save_path=feature_eng_data,
        spark_master=spark_master,
        spark_app_name=spark_app_name,
        run=run,
    )
    ...
    data_pipeline(
        feature_engineered_data_path=feature_eng_data,
        path_proc_data_path=path_proc_data_path,
        path_pipeline_model=path_pipeline_model,
        spark_master=spark_master,
        spark_app_name=spark_app_name,
        run=run,
    )
    ...
    run_model_experiment(
        model_experimenter=model_experiment, proc_data=processed_data_dir
    )
    ...
    log_wandb_artifact("model_explainability", artifacts_path, models_path, run)
